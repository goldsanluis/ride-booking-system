...
BG_ENTRY     = "#6b0000"   # Input/track background
GOLD         = "#FFD700"   # PUP Gold
TEXT_WHITE   = "#FFFFFF"   # White text
TEXT_GRAY    = "#ffcccc"   # Muted light for subtitles
RED          = "#FF6B6B"   # Error/cancel red
GREEN        = "#90EE90"   # Light green for completed
TEAL         = "#87CEEB"   # Light blue for scheduled/active
...
class DriverDashboard:
    def __init__(self, driver):
        self.driver         = driver
        self.root           = tk.Tk()
        self.root.title("PUP Rides — Driver Dashboard")
    ...
    def setup_header(self):
        ...
        # Top dark strip like login header
        tk.Frame(self.header_frame, bg=BG_DARK, height=6).pack(fill="x", pady=(0, 8))
    ...
    def setup_tabs(self):
        ...
        self.tab_rides_btn = tk.Button(
            ...
            font=("Helvetica", 11, "bold"), relief="flat", padx=20, pady=10,
    ...
    def _set_tab_styles(self, active):
        for btn, name in [(self.tab_rides_btn, "rides"),
                          (self.tab_earn_btn, "earnings"),
                          (self.tab_profile_btn, "profile")]:
            if name == active:
                btn.config(bg=BG_DARK, fg=GOLD, activebackground=BG_DARK)
            else:
    ...
    def _create_ride_card(self, booking):
        ...
        tk.Label(card, text=f"👤 {booking.user}",
                 font=("Helvetica", 12, "bold"), bg=BG_CARD, fg=TEXT_WHITE).pack(anchor="w")
        ...
        tk.Button(card, text="✅ Accept Ride",
                  font=("Helvetica", 11, "bold"), bg=GREEN, fg=BG_DARK,
    ...
    def accept_ride(self, booking):
        if messagebox.askyesno(
            ...
            self.earnings_label.config(text=f"  💰 ₱{self.driver['wallet_balance']:.2f}")
    ...
    def _build_earnings(self):
        ...
        stat(summary_row, "Avg per Ride",     f"₱{avg:.2f}",           GOLD)
        stat(summary_row, "Pending (active)", f"₱{pending:.2f}",       TEAL)
        ...
        tk.Label(fb, text="Filter:", font=("Helvetica", 9, "bold"),
                 bg=BG_CARD, fg=GOLD).grid(row=0, column=0, padx=(0, 6))
        ...
        st_cb.config(font=("Helvetica", 9), bg=BG_ENTRY, fg=TEXT_WHITE,
                     activebackground=BG_DARK, activeforeground=TEXT_WHITE,
                     relief="flat", bd=0, highlightthickness=0)
        st_cb["menu"].config(bg=BG_ENTRY, fg=TEXT_WHITE, font=("Helvetica", 9))
        ...
        pe_cb = tk.OptionMenu(fb, self.earn_period_var,
                              ...
                              command=lambda _: self._refresh_earnings_list(my_bookings))
        pe_cb.config(font=("Helvetica", 9), bg=BG_ENTRY, fg=TEXT_WHITE,
                     activebackground=BG_DARK, activeforeground=TEXT_WHITE,
                     relief="flat", bd=0, highlightthickness=0)
        pe_cb["menu"].config(bg=BG_ENTRY, fg=TEXT_WHITE, font=("Helvetica", 9))
    ...
    def _create_earnings_card(self, b):
        ...
        hdr.pack(fill="x")
        sc = GREEN if b.status == "Completed" else TEAL
        tk.Label(hdr, text=f"#{b.booking_id} • {b.user}",
                 font=("Helvetica", 10, "bold"), bg=BG_CARD, fg=TEXT_WHITE).pack(side="left")
        ...
        earn_color = GREEN if b.status == "Completed" else TEAL
    ...
    def _build_profile(self):
        ...
        tk.Label(avatar_frame, text=f"🚗 {d_data.get('plate', self.driver['plate'])}",
                 font=("Helvetica", 12), bg=BG_CARD, fg=TEXT_WHITE).pack()
        ...
        stat(stats_row, "📏", "Total Distance",  f"{total_km:.1f} km",   GOLD)
        stat(stats_row, "💳", "Avg per Ride",    f"₱{avg:.2f}",          GOLD)

        # Vehicle type breakdown
        tk.Label(inner, text="Rides by Vehicle Type",
                 font=("Helvetica", 11, "bold"), bg=BG_DARK, fg=GOLD).pack(pady=(16, 4))
        ...
        if by_v:
            ...
            for vname, count in sorted(by_v.items(), key=lambda x: x[1], reverse=True):
                pct = count / total_v
                row = tk.Frame(bar_frame, bg=BG_CARD)
